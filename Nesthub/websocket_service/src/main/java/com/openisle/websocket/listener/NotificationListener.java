    ...
    public void receiveMessage(MessageNotificationPayload payload, @Header("amqp_consumedQueue") @Nullable String queueName) {
        ...
        log.info("Payload content: {}", payloadObject);
            ...
            if (payloadObject instanceof Map) {
                ...
                // 处理聊天室弹窗通知
                if (payloadMap.containsKey("roomId") && payloadMap.containsKey("content") && payloadMap.containsKey("sender")) {
                    messagingTemplate.convertAndSend("/topic/chat-notification", payloadMap);
                    log.info("[ChatToast] 弹窗通知已推送到 /topic/chat-notification: roomId={}", payloadMap.get("roomId"));
                    return;
                }
