...
const CHAT_TOAST_SETTING_KEY = 'chat_toast_enabled'
const chatToastEnabled = ref(true)
...
onMounted(async () => {
  ...
  chatToastEnabled.value = localStorage.getItem(CHAT_TOAST_SETTING_KEY) !== 'false'
})
...
watch(chatToastEnabled, (val) => {
  localStorage.setItem(CHAT_TOAST_SETTING_KEY, val ? 'true' : 'false')
})
</script>
