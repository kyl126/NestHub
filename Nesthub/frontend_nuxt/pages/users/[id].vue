    ...
    <div v-else>
      <div class="profile-page-header">
        ...
        <div class="profile-page-header-user-info">
          ...
          <div class="profile-page-header-user-info-buttons">
            <!-- 添加好友按钮 -->
           <div
              v-if="!isMine && !isFriend && !friendRequestSent"
      ...
      <BaseTabs v-model="selectedTab" :tabs="tabs">
        ...
        <template v-else>
          ...
          <div v-else-if="selectedTab === 'friends'" class="friends-container">
            <div v-if="friends.length > 0">
            <UserList :users="friends" />
            </div>
            <div v-else>
            <BasePlaceholder text="暂无好友" icon="user" />
            </div>
          </div>
        </template>  
      </BaseTabs>
...
const checkFriendStatus = async () => {
  ...
  try {
    ...
    isFriend.value = friendsRes.some(f => f.id === user.value.id)
    
    // 检查是否已发送申请
    const requestsRes = await $fetch(`${API_BASE_URL}/api/friends/requests/sent`, {
      headers: { Authorization: `Bearer ${token}` }
    })
    friendRequestSent.value = requestsRes.some(r => r.receiver.id === user.value.id && r.status === 'PENDING')
  } catch (e) {
...
const sendFriendRequest = async () => {
  try {
    ...
    await $fetch(`${API_BASE_URL}/api/friends/request/${user.value.username}`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${token}` },
      body: { message: '请求添加你为好友' }
    })
    toast.success('好友申请已发送')
    friendRequestSent.value = true
    window.dispatchEvent(new Event('friend-request-sent'))
  } catch (e) {
    toast.error(e.data?.error || '发送失败')
  }
}

// 删除好友
const deleteFriend = async () => {
  if (!confirm('确定删除好友吗？')) return
  try {
    const token = getToken()
    await $fetch(`${API_BASE_URL}/api/friends/${user.value.username}`, {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${token}` }
    })
    toast.success('已删除好友')
...
const selectedTab = ref(
  ['summary', 'friends','timeline', 'following', 'favorites', 'achievements'].includes(route.query.tab)
    ? route.query.tab
    : 'summary',
)
...
const init = async () => {
  ...
  try {
    ...
    if (selectedTab.value === 'summary') {
      ...
      await loadTimeline()
    } else if (selectedTab.value === 'friends') {
      await loadFriends()
...
watch(selectedTab, async (val) => {
  ...
  else if (val === 'friends' && friends.value.length === 0) {
    await loadFriends()
  }
...
onMounted(() => {
  init()
  window.addEventListener('friend-status-changed', checkFriendStatus)
})

onUnmounted(() => {
  window.removeEventListener('friend-status-changed', checkFriendStatus)
})
</script>
