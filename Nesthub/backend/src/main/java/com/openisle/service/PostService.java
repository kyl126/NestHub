...
public class PostService {
  ...
  @CacheEvict(value = CachingConfig.POST_CACHE_NAME, allEntries = true, beforeInvocation = true)
  ...
  @CacheEvict(value = CachingConfig.POST_CACHE_NAME, allEntries = true)
    ...
    if ("官方".equals(subChannel)) {
      User author = userRepository.findByUsername(username)
          .orElseThrow(() -> new com.openisle.exception.NotFoundException("User not found"));
      if (author.getRole() != Role.ADMIN) {
        throw new IllegalArgumentException("官方子频道仅管理员可发帖");
      }
    }
    // 限制访问次数
    ...
    User author = userRepository
      .findByUsername(username)
        .orElseThrow(() -> new com.openisle.exception.NotFoundException("User not found"));
          if (author.isBanned()) {
      throw new IllegalArgumentException("您已被封禁，无法发帖");
    }
    if (author.getMutedUntil() != null && author.getMutedUntil().isAfter(LocalDateTime.now())) {
      throw new IllegalArgumentException("您已被禁言，无法发帖");
    }
    ...
    post.setSubChannel(subChannel != null && !subChannel.isEmpty() ? subChannel : "最新");
    ...
    // ========== 2. 根据帖子类型注册定时结算任务 ==========
    // 抽奖帖：到期自动执行 finalizeLottery 随机抽取中奖者
    if (post instanceof LotteryPost lp && lp.getEndTime() != null) {
      ...
      // 提案帖：到期自动执行 finalizeProposal 统计赞成率并决定是否通过
      ScheduledFuture<?> future = taskScheduler.schedule(
        () -> applicationContext.getBean(PostService.class).finalizeProposal(cp.getId()),
        java.util.Date.from(cp.getEndTime().atZone(ZoneId.systemDefault()).toInstant())
      );
      scheduledFinalizations.put(cp.getId(), future);
       ...
       // 投票帖：到期自动执行 finalizePoll 公布投票结果
       ScheduledFuture<?> future = taskScheduler.schedule(
           () -> applicationContext.getBean(PostService.class).finalizePoll(pp.getId()),
           java.util.Date.from(pp.getEndTime().atZone(ZoneId.systemDefault()).toInstant()));
       scheduledFinalizations.put(pp.getId(), future);
     }
      // ========== 3. 发布成功后加入搜索引擎索引 + 记录发帖频率 ==========
  ...
  public List<Post> listRecentPosts(int minutes) {
    ...
    return listPostsByViews(null, null, page, pageSize, null);
  }
  ...
  public List<Post> listPostsByViews(
    ...
    if (subChannel != null && !subChannel.isEmpty()) {
      return listPostsBySubChannel(categoryIds, tagIds, page, pageSize, subChannel);
    }
    ...
    return listPostsByLatestReply(null, null, page, pageSize, null);
  }
   ...
   */
    ...
    if (subChannel != null && !subChannel.isEmpty()) {
      return listPostsBySubChannel(ids, tids, page, pageSize, subChannel);
    }
  ...
  private List<Post> listPostsBySubChannel(
        List<Long> categoryIds,
        List<Long> tagIds,
        Integer page,
        Integer pageSize,
    ...
    log.info("listPostsBySubChannel: subChannel='{}', hasCategories={}",
            subChannel, categoryIds != null && !categoryIds.isEmpty());

    Pageable pageable = buildPageable(page, pageSize);
    boolean hasCategories = categoryIds != null && !categoryIds.isEmpty();

    // 统一预加载分类列表
    List<Category> categories = hasCategories
    ...
    // 子频道为空时默认"最新"
    String channel = (subChannel == null || subChannel.isEmpty()) ? "最新" : subChannel;

    List<Post> posts;

    switch (channel) {
        case "火" -> {
            // 全量查询，排除加精帖，按置顶 > 浏览量倒序 > 时间倒序
            posts = hasCategories
                    ? postRepository.findByCategoryInAndStatusOrderByPinnedAtDescViewsDesc(
                            categories, PostStatus.PUBLISHED, pageable)
                    : postRepository.findByStatusOrderByPinnedAtDescViewsDesc(
                    PostStatus.PUBLISHED, pageable);
            log.info("火查询原始数量: {}", posts.size());
            posts = posts.stream()
                    .filter(p -> p.getRssExcluded() == null || p.getRssExcluded())
                .toList();
            log.info("火过滤后数量: {}", posts.size());
        }
        case "精华" -> {
            // 精确匹配 subChannel = "精华" 或 全量加精帖，按发布时间倒序
            posts = hasCategories
                    ? postRepository.findByCategoryInAndSubChannelOrderByPinnedAndFeaturedAndTime(
                            categories, PostStatus.PUBLISHED, "精华", pageable)
                    : postRepository.findBySubChannelOrderByPinnedAndFeaturedAndTime(
                            PostStatus.PUBLISHED, "精华", pageable);
        }
        case "官方" -> {
            // 精确匹配 subChannel = "官方"，按置顶 > 发布时间倒序
            posts = hasCategories
                    ? postRepository.findByCategoryInAndSubChannelOrderByPinnedAndFeaturedAndTime(
                            categories, PostStatus.PUBLISHED, "官方", pageable)
                    : postRepository.findBySubChannelOrderByPinnedAndFeaturedAndTime(
                            PostStatus.PUBLISHED, "官方", pageable);
        }
        case "最新" -> {
            // 全量查询，排除加精帖，按置顶 > 发布时间倒序
            posts = hasCategories
                    ? postRepository.findByCategoryInAndStatusOrderByPinnedAndFeaturedAndTime(
                            categories, PostStatus.PUBLISHED, pageable)
                    : postRepository.findAllByStatusOrderByPinnedAndFeaturedAndTime(
                            PostStatus.PUBLISHED, pageable);
            log.info("最新查询原始数量: {}", posts.size());  // ← 加这行
            posts = posts.stream()
                    .filter(p -> p.getRssExcluded() == null || p.getRssExcluded())
                    .toList();
            log.info("最新过滤后数量: {}", posts.size());     // ← 加这行
        }
        default -> {
            // 自定义子频道：精确匹配 subChannel，按置顶 > 发布时间倒序
            posts = hasCategories
                    ? postRepository.findByCategoryInAndSubChannelOrderByPinnedAndFeaturedAndTime(
                            categories, PostStatus.PUBLISHED, channel, pageable)
                    : postRepository.findBySubChannelOrderByPinnedAndFeaturedAndTime(
                            PostStatus.PUBLISHED, channel, pageable);
        }
    }
    ...
    return paginate(posts, page, pageSize);
      ...
      @CacheEvict(value = CachingConfig.POST_CACHE_NAME, allEntries = true)
    @Transactional
    public void batchMoveCategory(List<Long> postIds, Long targetCategoryId) {
