...
public class AdminPostController {
  ...
  public PostSummaryDto includeInRss(
    ...
    @PutMapping("/batch-move-category")
    @SecurityRequirement(name = "JWT")
    public ResponseEntity<?> batchMoveCategory(@RequestBody Map<String, Object> payload) {
        List<Long> postIds = ((List<Integer>) payload.get("postIds"))
            .stream()
            .map(Long::valueOf)
            .collect(Collectors.toList());
        Long targetCategoryId = Long.valueOf(payload.get("targetCategoryId").toString());
        postService.batchMoveCategory(postIds, targetCategoryId);
