...
public class PostController {
  ...
  @Cacheable(
      ...
      key = "{ 'default', #categoryId, #categoryIds, #tagId, #tagIds, #page, #pageSize, #subChannel }"
  )
    ...
    return postMapper.toListDtos(postService.defaultListPosts(ids, tids, page, pageSize, subChannel));
  }
  ...
  @ApiResponse(
    ...
    List<Long> ids = categoryService.getSearchCategoryIds(categoryIds, categoryId);
    List<Long> tids = tagService.getSearchTagIds(tagIds, tagId);
    return postMapper.toListDtos(postService.listPostsByViews(ids, tids, page, pageSize, subChannel));
  }
  ...
  @Cacheable(
    value = CachingConfig.POST_CACHE_NAME,
    key = "{ 'latest_reply', #categoryId, #categoryIds, #tagIds, #page, #pageSize, #subChannel }"
  )
    ...
    List<Long> ids = categoryService.getSearchCategoryIds(categoryIds, categoryId);
    List<Long> tids = tagService.getSearchTagIds(tagIds, tagId);
    return postMapper.toListDtos(postService.listPostsByLatestReply(ids, tids, page, pageSize, subChannel));
  }
  ...
  @ApiResponse(
    ...
    List<Long> ids = categoryService.getSearchCategoryIds(categoryIds, categoryId);
    List<Long> tids = tagService.getSearchTagIds(tagIds, tagId);
    return postMapper.toListDtos(postService.listFeaturedPosts(ids, tids, page, pageSize, subChannel));
  }
